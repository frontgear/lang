...
var _registerClassName = function(cls, name, isNative)
{
	...
	if (isNative)
		_classNativeCount++;
	else if (Class.isFunction(cls) && Class.isFunction(_defineProperty))
		_defineProperty(cls, "name", { value: name, writable: false, enumerable: false, configurable: true });
};
...
Class.getName = function(cls)
{
	...
	var name = Class.isFunction(cls) ? cls.name : null;
	if (name && Class.isString(name))
		return name;
...
_registerClassName(Class, "@frontgear/lang/Class");
